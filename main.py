from tkinter import (Tk, Button, Frame, Canvas, W, E, X, filedialog, messagebox, colorchooser)
import PIL.Image
...
class AppGUI:
    def __init__(self):
        ...
        self.brush_minus_btn = Button(self.btn_frame, text="B-", command=self.brush_minus)
        self.brush_minus_btn.grid(row=1, column=0, sticky=W+E)

        self.color_btn = Button(self.btn_frame, text="Change Color", command=self.pick_color)
    ...
    def paint(self, e):
        ...
        self.cnv.create_rectangle(x0, y0, x1, y1, outline=self.curcolor, fill=self.curcolor, width=self.brush_width)
        self.draw.ellipse([x0, y0, x1 + self.brush_width, y1 + self.brush_width], outline=self.curcolor, fill=self.curcolor, width=self.brush_width)
